...
const BALL_SIZE = 80
const BALL_LEN = 20
const list = [10, 15, 20, 25, 30, 35, 50]
const NAME = 'KL8'
const __dirname = dirname(fileURLToPath(import.meta.url))
const dataPath = resolve(__dirname, `./kl8.json`)
...
writeFileSync(
  ...
  // 格式二选一：选一种你需要的格式
  sortedTimes.join('\n'), // 格式1：文本格式（每行一个，清晰）
  // JSON.stringify(sortedTimes, null, 2), // 格式2：JSON数组格式（便于后续解析）
  'utf-8',
)

function main(windowSize) {
  const outputPath = resolve(__dirname, `${NAME}${windowSize}.txt`)
  const outputPathTotal = resolve(__dirname, `${NAME}_TOTAL${windowSize}.txt`)
  try {
    // 1. 读取并验证数据
